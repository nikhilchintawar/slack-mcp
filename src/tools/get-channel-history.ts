...
import type { ChannelHistoryResult, SlackMessage } from '../types.js';
...
export const getChannelHistorySchema = z.object({
  channel: z
    ...
    .optional()
    .describe(
      'Channel ID to fetch history from. Uses SLACK_DEFAULT_CHANNEL if not provided.'
    ),
  oldest: z
    .string()
    .optional()
...
export async function getChannelHistory(
  ...
  if (!channelId) {
    throw new Error(
      'Channel ID is required. Provide it as a parameter or set SLACK_DEFAULT_CHANNEL.'
    );
  }
  ...
  do {
    ...
    if (response.messages) {
      for (const msg of response.messages) {
        ...
        messages.push(mapMessage(msg as SlackHistoryMessage, channelId));
      }
    }
    ...
    for (const threadParent of threadMessages) {
      ...
      if (threadResponse.messages) {
        ...
        for (let i = 1; i < threadResponse.messages.length; i++) {
          ...
          messages.push(mapMessage(reply as SlackHistoryMessage, channelId));
        }
      }
    }
  }
